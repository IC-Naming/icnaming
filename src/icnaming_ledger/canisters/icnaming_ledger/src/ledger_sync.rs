...
use crate::canisters::ledger::get_ledger_id;
...
async fn sync_transactions_within_lock() -> Result<u32, String> {
    ...
    if block_height_synced_up_to.is_none() {
        ...
        STATE.with(|s| {
            let mut store = s.payments_store.borrow_mut();
    ...
    if tip_of_chain < next_block_height_required {
        ...
        STATE.with(|s| s.payments_store.borrow_mut().mark_ledger_sync_complete());
        Ok(0)
    } else {
...
fn determine_canister_for_blocks(
    ...
    (get_ledger_id(), from..=tip_of_chain)
}
