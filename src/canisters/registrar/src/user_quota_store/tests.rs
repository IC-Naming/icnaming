...
#[fixture]
fn user_quota_manager() -> UserQuotaStore {
    UserQuotaStore::new()
}

#[rstest]
fn test_get_user_quota_not_set(user_quota_manager: UserQuotaStore, owner: Principal) {
    let user_quota = user_quota_manager.get_quota(&owner.clone(), &QuotaType::LenGte(4));
    assert_eq!(user_quota, None);
}

#[rstest]
fn test_add_quota(mut user_quota_manager: UserQuotaStore, owner: Principal) {
    ...
    for i in 0..len {
        ...
        user_quota_manager.add_quota(owner.clone(), quota_type.clone(), (i + 1) as u32);
        user_quota_manager.add_quota(owner.clone(), quota_type.clone(), (i + 1) as u32);
    }
...
#[rstest]
fn test_sub_quota(mut user_quota_manager: UserQuotaStore, owner: Principal) {
