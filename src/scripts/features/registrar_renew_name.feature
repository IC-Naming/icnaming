...
Feature: Renew a name
  ...
  Scenario: Renew a name successfully
    Given User "user1" register name "7654321.ark" with quote "LenGte(3)"
    And registrar get_details "7654321.ark" result is
      | key        | value       |
      | owner      | user1       |
      | name       | 7654321.ark |
      | expired_at | 1           |
      | created_at | 0           |
    When User "user1" renew name "7654321.ark" for "3" years and pay "3 icp"
  ...
  Scenario: Renew a name successfully for 5-chars name
    ...
    And registrar get_details "hello.ark" result is
      ...
      | expired_at | 1         |
      | created_at | 0         |
    When User "user1" renew name "hello.ark" for "3" years and pay "7.26 icp"
    Then Last renew name status is "Ok"
    And registrar get_details "hello.ark" result is
      | key        | value     |
      | owner      | user1     |
  ...
  Scenario: Renew a name success with 95% lower price
    ...
    And registrar get_details "7654321.ark" result is
      ...
      | expired_at | 1           |
      | created_at | 0           |
    When User "user1" renew name "7654321.ark" for "5" years and pay "4.75 icp"
    Then Last renew name status is "Ok"
    And registrar get_details "7654321.ark" result is
      | key        | value       |
      | owner      | user1       |
  ...
  Scenario: Renew a name failed since too many years
    ...
    And registrar get_details "7654321.ark" result is
      ...
      | expired_at | 1           |
      | created_at | 0           |
    When User "user1" renew name "7654321.ark" for "11" years and pay "11 icp"
  ...
  Scenario: Renew a name failed since not enough money
    ...
    And registrar get_details "7654321.ark" result is
      ...
      | expired_at | 1           |
      | created_at | 0           |
    When User "user1" renew name "7654321.ark" for "10" years and pay "3 icp"
  ...
  Scenario: Renew a name failed since caller is not owner
    ...
    When User "user2" renew name "7654321.ark" for "5" years and pay "5 icp"
