...
Feature: Registrar Control Gateway
  ...
  Scenario: Assign a reserved name success
    ...
    When admin assign name "icnaming.ic" to user "user1"
    And admin assign name "icp.ic" to user "user2"
    Then last assign name status is "Ok"
    And registrar get_details "icnaming.ic" result is
      | key        | value        |
      | owner      | user1        |
      | name       | icnaming.ic |
      | expired_at | 1            |
      | created_at | 0            |
    And registrar get_details "icp.ic" result is
      | key        | value   |
      | owner      | user2   |
      | name       | icp.ic |
      | expired_at | 1       |
  ...
  Scenario: Assign a name more than once, and owner should be the first one
    Given Update quota as follow operations
      | operation | user                      | quota_type1 | quota_type2 | value |
      | add       | registrar_control_gateway | LenGte      | 1           | 10    |
    And admin assign name "icnaming.ic" to user "user1"
    And last assign name status is "Ok"
    When admin assign name "icnaming.ic" to user "user2"
  ...
  Scenario: Assign a name without quota result in fail
    Given admin assign name "icnaming.ic" to user "user1"
  ...
  Scenario: Assign a reserved name success and transfer by admin to another user
    Given Update quota as follow operations
      ...
      | add       | registrar_control_gateway | LenGte      | 1           | 10    |
    And admin assign name "icnaming.ic" to user "user1"
    When User "main" transfer name "icnaming.ic" to user "user3"
  ...
  Scenario: Failed to transfer_by_admin if is not admin
    Given Update quota as follow operations
      ...
      | add       | registrar_control_gateway | LenGte      | 1           | 10    |
    And admin assign name "icnaming.ic" to user "user1"
    When User "user1" transfer name "icnaming.ic" to user "user3"
