...
Feature: Name Transaction
  ...
  Scenario: Transfer name by user
    Given User "user1" register name "hello.ark" with quote "LenGte(3)"
    And User "user1" update resolver "hello.ark" with values
      | key          | value                                      |
    ...
    When User "user1" transfer name "hello.ark" to User "user2"
    ...
    And registrar get_details "hello.ark" result is
      ...
      | expired_at | 1         |
      | created_at | 0         |
    And get_record_value "hello.ark" should be as below
      | key | value |
  ...
  Scenario: Transfer name twice
    Given User "user1" register name "hello.ark" with quote "LenGte(3)"
    When User "user1" transfer name "hello.ark" to User "user2"
    And User "user2" update resolver "hello.ark" with values
      | key          | value                                      |
      | token.ark    | qjdve-lqaaa-aaaaa-aaaeq-cai                |
      | token.btc    | 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa         |
      | token.ltc    | LUwxSibYhxq2u6RfhQmkuTPZRk2wNjwLbE         |
      | token.eth    | 0xb436ef6cc9f24193ccb42f98be2b1db764484514 |
      | canister.ark | qsgjb-riaaa-aaaaa-aaaga-cai                |
    And User "user2" transfer name "hello.ark" to User "user3"
    Then last name transfer result status is "Ok"
    And registrar get_details "hello.ark" result is
      | key        | value     |
      | owner      | user3     |
  ...
  Scenario: Transfer name fail without permission
    Given User "user1" register name "hello.ark" with quote "LenGte(3)"
    When User "user2" transfer name "hello.ark" to User "user3"
  ...
  Scenario: Transfer name by transfer_from
    ...
    And User "user1" update resolver "hello.ark" with values
      ...
      | token.eth    | 0xb436ef6cc9f24193ccb42f98be2b1db764484514 |
      | canister.ark | qsgjb-riaaa-aaaaa-aaaga-cai                |
    And User "user1" approve name "hello.ark" to User "user2"
    When User "user2" transfer name "hello.ark" by transfer_from
  ...
  Scenario: Removing approval manually
    Given User "user1" register name "hello.ark" with quote "LenGte(3)"
    And User "user1" approve name "hello.ark" to User "user2"
    When User "user1" approve name "hello.ark" to User "anonymous"
    And User "user2" transfer name "hello.ark" by transfer_from
    Then last name transfer_from result status is "permission deny"

  Scenario: Removing approval after transfer
    Given User "user1" register name "hello.ark" with quote "LenGte(3)"
    And User "user1" approve name "hello.ark" to User "user2"
    And User "user1" transfer name "hello.ark" to User "user3"
    When User "user3" transfer name "hello.ark" by transfer_from
    Then last name transfer_from result status is "permission deny"
  ...
  Scenario: Reclaim name successfully if owner of registry is changed
    ...
    And User "user1" update resolver "hello.ark" with values
      ...
      | token.ark | qjdve-lqaaa-aaaaa-aaaeq-cai        |
      | token.btc | 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa |
    And User "user1" set registry owner for "hello.ark" to "user2"
    When User "user1" reclaim name "hello.ark"
    And registrar get_details "hello.ark" result is
      | key        | value     |
      | owner      | user1     |
      | name       | hello.ark |
