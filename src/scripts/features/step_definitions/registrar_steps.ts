...
Then(/^registrar get_details "([^"]*)" result is$/,
    async function (name: string, data) {
        ...
        const identityInfo = identities.get_identity_info(target.owner);
...
When(/^Update quota as follow operations$/,
    async function (data) {
        ...
        for (const op of operations) {
            ...
            const user_principal = identities.get_principal(op.user);
            if (op.operation === "add") {
...
Then(/^User quota status should be as below$/,
    async function (data) {
        ...
        for (const item of items) {
            ...
            const user_principal = identities.get_principal(item.user)
...
When(/^Do quota transfer as below$/,
    async function (data: DataTable) {
        ...
        for (const item of items) {
            const quota_type = {};
            quota_type[item.quota_type1] = parseInt(item.quota_type2);
            const to_principal = get_principal(item.to);
            const registrar = createRegistrar(identities.get_identity_info(item.from));
...
Given(/^Some users already have some quotas$/,
    async function (data) {
        ...
        for (const item of items) {
            const quota_type = {};
            quota_type[item.quota_type1] = parseInt(item.quota_type2);
            const user_principal = identities.get_identity_info(item.user).identity.getPrincipal();
...
When(/^Do quota transfer_from_quota as below by admin$/,
    async function (data: DataTable) {
        ...
        for (const item of items) {
            ...
            quota_type[item.quota_type1] = parseInt(item.quota_type2);
            const to_principal = get_principal(item.to);
            const from_principal = get_principal(item.from);
...
Then(/^I found my pending order as bellow$/,
    async function (data: DataTable
        ...
        const target_type = to_quota_type(rows.quota_type);
...
Then(/^Get last registrations result is$/,
    async function (data: DataTable) {
        ...
        const registrar = createRegistrar(identities.timer_trigger);
